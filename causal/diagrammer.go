...
* Feedback Loop: A set of Causal Relationships that form a cycle in the Causal Loop Diagram graph.  We sometimes call a set of causal relationships that form a cycle a "closed" feedback loop.  Feedback loops are THE critical feature of causal loop diagrams - they describe the endogenous structure that drives the behavior of a system.  If a CLD doesn't contain feedback loops, then it doesn't contain an explanation for the behavior of the system.  A chain of causal relationships that doesn't end at the first variable by definition isn't a loop.  In our example, there is a feedback loop that goes "Births", "Population", "Births": an increase in births increases the total population, which further increases births (as there are more breeding individuals).  A chain of relationships like "Death Rate" to "Deaths" to "Population" is NOT a feedback loop, as it does not end on (loop back to) the first variable "Death Rate".   
