...
const __exename = path.basename(__dirname);
...
class Engine {
    ...
    static supportedModes() {
        // check that the Go binary exists
        try {
            const stats = fs.statSync(`${__dirname}/${__exename}`);
    ...
    async generate(prompt, currentModel, parameters) {
        ...
        let tempDir;
        try {
            tempDir = fs.mkdtempSync(path.join(tmpdir(), 'sd-ai-stock-and-flow-'));
            ...
            const result = execSync(`${__dirname}/${__exename} ${inputPath}`, {cwd: tempDir});

            return JSON.parse(result.toString());
        } catch (err) {
            console.log(`${__dirname} returned non-zero exit code: ${err.status}`);
            return {
